...
{
    /// <inheritdoc/>
    public (byte[] data, string ext) GetNotificationLogoData(byte[] logoData, string extLogo, TenantWhiteLabelSettings tenantWhiteLabelSettings)
    {
        ...
        return extLogo switch
        {
            ...
            _ => (null, extNotification)
        };

        byte[] GetLogoDataFromSvg()
        {
            var size = TenantWhiteLabelSettings.GetSize(WhiteLabelLogoType.Notification);
