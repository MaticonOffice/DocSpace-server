    ...
    IFusionCache hybridCache)
{
    ...
    {
        var configured = configuration.GetSection("web:logo:custom-cultures").Get<string[]>() ?? [];
    ...
    {
        var normalizedCulture = culture?.Replace('_', '-') ?? string.Empty;
        if (normalizedCulture.StartsWith("ru", StringComparison.InvariantCultureIgnoreCase))
        {
            return ".ru";
        }
        ...
        return customCultures.Contains(normalizedCulture, StringComparer.InvariantCultureIgnoreCase)
