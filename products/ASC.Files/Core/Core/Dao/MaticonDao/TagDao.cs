// Copyright (C) Maticon Office LLC, 2025-2026
// 
...
{
    private const int MaxRecentTagsCount = 1000;
    public async IAsyncEnumerable<Tag> GetTagsAsync(Guid subject, IEnumerable<TagType> tagType, IEnumerable<FileEntry<T>> fileEntries)
    {
        ...
        {
            var (id, _) = await mapping.MappingIdAsync(f.Id);
        ...
        {
            yield return await ToTagAsync(filesDbContext, tenantId, e);
        }
    }
    ...
    {
        var mapping = daoFactory.GetMapping<T>();
        var (mappedId, _) = await mapping.MappingIdAsync(entryId);
    ...
    public async IAsyncEnumerable<TagInfo> GetTagsInfoAsync(IEnumerable<string> names, TagType type)
    {
        ...
        {
            yield return await ToTagAsyncInfo(filesDbContext, tenantId, tag);
        }
    }
            ...
            await strategy.ExecuteAsync(async () =>
            {
                ...
                foreach (var t in tags)
                {
                    ...
                    var linkToInsert = new DbFilesTagLink
                    {
                        TenantId = tenantId,
                        TagId = toAdd.Id,
                        EntryId = int.TryParse(t.EntryId.ToString(), out var n) ? n.ToString() : (await MappingIdAsync(internalFilesDbContext, tenantId, t.EntryId, true)).ToString(),
    ...
    public async Task<IEnumerable<Tag>> SaveTagsAsync(Tag tag, Guid createdBy = default)
    {
        var result = new List<Tag>();
        ...
        await using (await _distributedLockProvider.TryAcquireLockAsync(GetLockKey(tenantId)))
        {
            ...
            await strategy.ExecuteAsync(async () =>
            {
                ...
                if (tag.Type is TagType.Recent or TagType.RecentByLink && createdBy != Guid.Empty)
                {
                    await internalFilesDbContext.Tag.Where(r => r.TenantId == tenantId)
                        .Join(internalFilesDbContext.TagLink, r => r.Id, l => l.TagId, (t, l) => new TagLinkData { Tag = t, Link = l })
                        .Where(r => r.Link.TenantId == r.Tag.TenantId)
                        .Where(r => (r.Tag.Type == TagType.Recent || r.Tag.Type == TagType.RecentByLink) && r.Link.CreateBy == createdBy)
                        .Select(r => r.Link)
                        .OrderByDescending(r => r.CreateOn)
                        .Skip(MaxRecentTagsCount - 1)
                        .ExecuteDeleteAsync();
                }
    ...
    private async Task<Tag> SaveTagAsync(FilesDbContext filesDbContext, int tenantId, Tag t, Dictionary<string, int> cacheTagId, DateTime createOn, Guid createdBy = default)
    {
        ...
        var linkToInsert = new DbFilesTagLink
        {
            TenantId = tenantId,
            TagId = id,
            EntryId = (await MappingIdAsync(filesDbContext, tenantId, t.EntryId, true)).ToString(),
            EntryType = t.EntryType,
            CreateBy = createdBy != Guid.Empty ? createdBy : _authContext.CurrentAccount.ID,
            CreateOn = createOn,
            Count = t.Count
    ...
    public async Task IncrementNewTagsAsync(IEnumerable<Tag> tags, Guid createdBy = default)
    {
        ...
        await using (await _distributedLockProvider.TryAcquireLockAsync(GetLockKey(tenantId), TimeSpan.FromMinutes(5)))
        {
            ...
            await strategy.ExecuteAsync(async () =>
            {
                ...
                {
                    var mappedId = (await MappingIdAsync(internalFilesDbContext, tenantId, tagsGroup.Key.EntryId)).ToString();
    ...
    public async Task<int> RemoveTagLinksAsync(T entryId, FileEntryType entryType, TagType tagType)
    {
        var tenantId = _tenantManager.GetCurrentTenantId();
        await using var filesDbContext = await _dbContextFactory.CreateDbContextAsync();
        var (mappedId, _) = await daoFactory.GetMapping<T>().MappingIdAsync(entryId);
    ...
    private ValueTask<object> MappingIdAsync(FilesDbContext filesDbContext, int tenantId, object id, bool saveIfNotExist = false)
    {
        if (id == null)
        {
        ...
        return InternalMappingIdAsync(filesDbContext, tenantId, id, saveIfNotExist);
    }

    private async ValueTask<object> InternalMappingIdAsync(FilesDbContext filesDbContext, int tenantId, object id, bool saveIfNotExist = false)
    {
        object result;
          ...
          socketManager)
{
    ...
    private async IAsyncEnumerable<Tag> InternalGetNewTagsAsync(Guid subject, Folder<int> parentFolder, bool deepSearch)
    {
        ...
        await using (var filesDbContext = await _dbContextFactory.CreateDbContextAsync())
        {
            foreach (var e in fromDb)
            {
                var tag = await ToTagAsync(filesDbContext, tenantId, e);
                yield return tag;
