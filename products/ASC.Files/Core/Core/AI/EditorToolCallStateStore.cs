...
[Scope]
public class EditorToolCallStateStore(IFusionCache cache)
{
    private static readonly TimeSpan _cacheExpiration = TimeSpan.FromMinutes(10);

    public async Task SetAsync(int fileId, EditorToolCallState callState)
    {
        var cacheKey = GetCacheKey(fileId);
        await cache.SetAsync(cacheKey, callState, _cacheExpiration);
    }

    public async Task<EditorToolCallState?> GetAsync(int fileId)
    {
        var cacheKey = GetCacheKey(fileId);
        return await cache.GetOrDefaultAsync<EditorToolCallState>(cacheKey);
    }

    private static string GetCacheKey(int fileId)
    {
        return $"ai:editor_tool_call_state:{fileId}";
    }
}
