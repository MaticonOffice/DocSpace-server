...
public class RoomNotifyQueueManager<T>(NotifyClient notifyClient, TenantManager tenantManager, FileSecurity fileSecurity) : INotifyQueueManager<T>
{
    private readonly ConcurrentDictionary<string, IRoomNotifyQueue<T>> _queues = new();
    ...
    public IRoomNotifyQueue<T> GetOrCreateRoomQueue(int tenantId, Folder<T> room, Guid currentAccountId)
    {
        ...
        {
            var roomNotifyQueue = new RoomNotifyQueue<T>(tenantId, room, notifyClient, currentAccountId, tenantManager, fileSecurity);
