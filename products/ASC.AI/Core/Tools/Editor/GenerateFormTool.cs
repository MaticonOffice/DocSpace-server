...
[Scope]
public class GenerateFormTool(IToolCallReceiver toolCallReceiver, ToolCallWaiterRegistry waiterRegistry)
{
        ...
        Task OnToolCall(FunctionCallContent call, CancellationToken cancellationToken)
        {
            return EditorToolInvoker.SubscribeAsync(toolCallReceiver, waiterRegistry, resultStorageId, userId, Extension, call,
                args => new EditorToolCallState
                {
                    ...
                    Parameters = new GenerateFormToolCallParameters { Description = EditorToolInvoker.GetArg(args, "description") ?? string.Empty }
                },
                cancellationToken);
        }
    }
