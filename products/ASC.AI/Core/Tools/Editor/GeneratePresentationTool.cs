...
[Scope]
public class GeneratePresentationTool(IToolCallReceiver toolCallReceiver, ToolCallWaiterRegistry waiterRegistry)
{
        ...
        Task OnToolCall(FunctionCallContent call, CancellationToken cancellationToken)
        {
            return EditorToolInvoker.SubscribeAsync(toolCallReceiver, waiterRegistry, resultStorageId, userId, Extension, call,
                args => new EditorToolCallState
                {
                    ...
                    {
                        Topic = EditorToolInvoker.GetArg(args, "topic"),
                        SlideCount = EditorToolInvoker.GetArg(args, "slideCount"),
                        Style = EditorToolInvoker.GetArg(args, "style")
                    }
                },
                cancellationToken);
        }
