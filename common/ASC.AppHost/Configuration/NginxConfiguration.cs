...
public static class NginxConfiguration
{
        ...
        bool isPreview = false)
    {
        ...
        openResty.WithArgs("/bin/sh", "-c",
            ...
            $"envsubst '{string.Join(' ', serviceUrls.Select(r => $"${r.Key}"))}' < /etc/nginx/includes/maticonoffice-upstream-map.conf.template > /etc/nginx/includes/maticonoffice-upstream-map.conf && " +
