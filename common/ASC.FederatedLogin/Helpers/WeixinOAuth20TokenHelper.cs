...
[Scope]
public class WeixinOAuth20TokenHelper(
    ...
    RequestHelper requestHelper) : IOAuth20TokenHelper
{
        ...
        where T : Consumer, IOAuthProvider, new()
    {
        ...
        {
            query += $"&appid={HttpUtility.UrlEncode(clientID)}";
        }
    ...
    public OAuth20Token GetAccessToken<T>(string authCode) where T : Consumer, IOAuthProvider, new()
    {
        ...
        var data = $"code={HttpUtility.UrlEncode(authCode)}&appid={HttpUtility.UrlEncode(clientID)}&secret={HttpUtility.UrlEncode(clientSecret)}";
        ...
        if (json != null)
        {
            ...
            var token = WeixinOAuth20Token.FromJson(json);
            if (token == null)
            {
                return null;
            }
    ...
    public OAuth20Token RefreshToken(string requestUrl, OAuth20Token token)
    {
        ...
        var data = $"appid={HttpUtility.UrlEncode(token.ClientID)}&refresh_token={HttpUtility.UrlEncode(token.RefreshToken)}&grant_type=refresh_token";

        var json = requestHelper.PerformRequest(requestUrl, "application/x-www-form-urlencoded", "POST", data);
        if (json != null)
        {
            var refreshed = WeixinOAuth20Token.FromJson(json);
