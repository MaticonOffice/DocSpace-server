...
/// <summary>
/// In-process heartbeat monitor using ConcurrentDictionary.
/// Designed for single-process (monolith/standalone) deployment mode.
/// </summary>
public class InMemoryHeartBeatMonitor : IHeartBeatMonitor
{
    public ValueTask<bool> IsAliveAsync(string key, CancellationToken cancellationToken = default)
    {
        return ValueTask.FromResult(InMemoryHeartBeatStore.Contains(key));
    }
}
