...
/**
 * No-op implementation of {@link AuthorizationMessagePublisher} for {@link
 * ClientCacheTenantRemoveEvent}.
 *
...
@Component
@Profile("!saas")
public class NoOpClientCacheTenantRemoveMessagePublisher
    implements AuthorizationMessagePublisher<ClientCacheTenantRemoveEvent> {

  /**
   * Logs the tenant-wide client cache removal event without publishing.
   *
   ...
   *
   * @param message the {@link ClientCacheTenantRemoveEvent} to log
   */
  @Override
  public void publish(ClientCacheTenantRemoveEvent message) {
    log.debug(
        "Skipping tenant-wide client cache removal event broadcast: tenantId={}",
        message.getTenantId());
  }
}
