...
public class PortalControllerHelper
{
    ...
    private readonly IDbContextFactory<MaticonSiteContext> _maticonSiteContext;

    public PortalControllerHelper(ILogger<PortalControllerHelper> log, TenantDomainValidator tenantDomainValidator, IDbContextFactory<MaticonSiteContext> maticonSiteContext)
    {
        _log = log;
        _tenantDomainValidator = tenantDomainValidator;
        _maticonSiteContext = maticonSiteContext;
    }
    ...
    public async Task AddTenantToCacheAsync(string tenant)
    {
        ...
        await using var context = _maticonSiteContext.CreateDbContext();
...
{
    public static readonly Func<MaticonSiteContext, string, Task<DbCache>> DbCacheAsync =
        EF.CompileAsyncQuery(
            (MaticonSiteContext ctx, string portalName) =>
                ctx.Cache.SingleOrDefault(q => q.TenantAlias == portalName));

    public static readonly Func<MaticonSiteContext, string, Task<bool>> AnyDbCacheAsync =
        EF.CompileAsyncQuery(
            (MaticonSiteContext ctx, string portalName) =>
                ctx.Cache
                    .Any(q => q.TenantAlias.Equals(portalName)));

    public static readonly Func<MaticonSiteContext, string, IAsyncEnumerable<string>> TenantAliasesAsync =
        EF.CompileAsyncQuery(
            (MaticonSiteContext ctx, string portalName) =>
                ctx.Cache
