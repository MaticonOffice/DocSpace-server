...
public class GenerateTypeScriptSdkCommand : SdkCommandBase
{
    ...
    private static int CopyPackages(string typeScriptSdkDirectory)
    {
        ...
        foreach (var packagePath in Directory.EnumerateFiles(typeScriptSdkDirectory, "maticonoffice-docspace-api-sdk-*.tgz"))
        {
        ...
        {
            AnsiConsole.MarkupLine(
                $"[red]{Markup.Escape($"No .tgz files matching 'maticonoffice-docspace-api-sdk-*.tgz' were found in '{typeScriptSdkDirectory}' after running npm pack.")}[/]");
            return 1;
        }
