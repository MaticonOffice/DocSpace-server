...
builder.Services.AddBaseDbContext<MaticonSiteContext>();
...
if (generateScript)
{
    ...
    else
    {
        ...
        foreach (var providerInfo in providersInfo.MaticonsiteProviders)
        {
            var runner = new MigrationRunner(app.Services);
            scripts.Add(runner.RunGenerateScript(providerInfo, configurationInfo, typeof(MaticonSiteContext), targetMigration));
        }
    }
...
else
{
    ...
    else
    {
        ...
        {
            var migrationCreator = new MigrationRunner(app.Services);
            migrationCreator.RunApplyMigrations(providerInfo, configurationInfo, typeof(MaticonSiteContext), targetMigration);
        }
    }
